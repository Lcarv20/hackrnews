...
const SheetCtx = createContext<SheetContextType>({
  isOpen: false,
  setIsOpen: () => {},
});
...
function by10px(el: HTMLElement, direction: "open" | "close" = "open") {
  ...
  const translation =
    direction === "open" ? currentTransform + 10 : currentTransform - 10;
...
export const Trigger = <C extends React.ElementType = "span">({
  ...
  return (
    <Component onClick={handleOpen} {...restProps}>
      {children}
    </Component>
  );
};
  ...
  return (
    ...
    >
      <ReactSheet.Container className="">
        <ReactSheet.Header />
        <ReactSheet.Content className="pb-4">{children}</ReactSheet.Content>
